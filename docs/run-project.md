...
2. Выбор питомца в разделе `Питомец`.
3. Создание, чтение, изменение и удаление привычки.
4. Отметка `completed`, `missed` и `recovery_completed`.
5. XP/level в gamification summary.
6. Генерация и чтение советов.
7. Dashboard на новой пустой базе без server error.
