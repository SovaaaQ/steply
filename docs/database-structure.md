...
- При `AUTO_INIT_DB=true` backend на старте выполняет `alembic upgrade head` через `init_db()` и затем idempotent seed справочников `achievements`/`quests`.
- Для legacy dev-баз, созданных до Alembic, `init_db()` распознает полный старый набор таблиц без `alembic_version`, добавляет только известные legacy-колонки и делает `alembic stamp head`. Неполная старая схема останавливает startup вместо молчаливого создания смеси таблиц.
...
|---|---|---|---|
| `users` / `User` | Аккаунт пользователя, базовый XP/уровень и выбранный питомец. | `id`, `email`, `full_name`, `hashed_password`, `experience_points`, `level`, `lives`, `pet_type`, `pet_name`, `pet_state`, `created_at` | 1:N к `habits`, `habit_entries`, `predictions`, `recommendations`, `reward_events`, `xp_history`; 1:1 к `user_gamification_profiles`; 1:N к `user_achievements`, `user_quest_progress`. |
...
| `quests` / `Quest` | Справочник заданий/миссий. | `id`, `type`, `tone`, `title`, `description`, `metric`, `target`, `target_metric`, `reward_xp`, `cta_label`, `cta_section`, `next_step`, `active_when`, `sort_order`, `is_active`, `created_at` | 1:N к `user_quest_progress`. |
| `user_quest_progress` / `UserQuestProgress` | Прогресс пользователя по заданиям за период. | `id`, `user_id`, `quest_id`, `period_key`, `progress`, `target`, `status`, `completed_at`, `reward_event_id`, `updated_at`; unique `user_id + quest_id + period_key` | N:1 к `users`, `quests`; optional N:1 к `reward_events`. |
| `reward_events` / `RewardEvent` | Журнал начислений XP и игровых событий. | `id`, `user_id`, `event_type`, `event_key`, `xp_amount`, `description`, `source_type`, `source_id`, `meta`, `created_at`; unique `user_id + event_key` | N:1 к `users`; referenced by achievements/quests progress. |
...
- Справочники прогресса защищены `uq_user_achievement`, `uq_user_quest_period`; idempotent игровые события защищены `uq_reward_event_user_key`.
- Для основных enum-like полей заданы check constraints: тип/сложность привычки, entry status, prediction risk, recommendation priority, pet type/state, streak/quest status, non-negative XP и счетчики.
...
- `HabitEntry -> XPHistory/RewardEvent -> UserGamificationProfile`: `xp_history` хранит habit-XP audit, а общий XP/level пересчитывается из idempotent `reward_events`, чтобы учитывать также достижения, задания и прочитанные советы.
...
- `Achievement/Quest -> UserAchievement/UserQuestProgress -> RewardEvent`: справочники достижений и заданий сидятся сервисом геймификации, пользовательский прогресс и награды хранятся отдельно.
...
- `backend/app/api/routes/gamification.py`: summary геймификации, обновление питомца, достижения, задания, история XP.
...
- `backend/app/services/gamification.py`: сидинг справочников, расчет метрик, XP, reward events, achievements, quests и профиля.
